...
from transformers.models.qwen2_5_vl.modeling_qwen2_5_vl import (
    BaseModelOutputWithPast as Qwen25BaseModelOutputWithPast,
)
...
from mmshift.utils.mmshift_util import (
    DynamicCacheSplitHeadFlatten,
    init_adakv,
    init_expected_attention,
    init_keydiff,
    init_mask,
    init_pyramidkv,
    init_shiftkv,
...
_POSITION_EMBEDDING_WARNING = (
    "The attention layers in this model are transitioning from computing the RoPE embeddings "
    "internally through `position_ids` (2D tensor with the indexes of the tokens), to using "
    "externally computed `position_embeddings` (Tuple of tensors, containing cos and sin). "
    "In v4.46 `position_ids` will be removed and `position_embeddings` will be mandatory."
)
_FLOAT32_CAST_WARNING = (
    "The input hidden states seems to be silently casted in float32, this might be related to "
    "the fact you have upcasted embedding or layer norm layers in float32. We will cast back "
    "the input in {target_dtype}."
)


def _project_qkv(self, hidden_states: torch.Tensor):
    """Project and reshape QKV tensors into attention-head layout."""
    ...
    return bsz, q_len, query_states, key_states, value_states


def _resolve_qwen_position_embeddings(
    self,
    ...
    *,
    ...
    """Resolve RoPE inputs, warning once when older position-id flow is used."""
    if position_embeddings is None:
        if warn_missing:
            logger.warning_once(_POSITION_EMBEDDING_WARNING)
        return self.rotary_emb(value_states, position_ids)
    return position_embeddings


def _cast_flash_attn_inputs(
    self,
    ...
    *,
    ...
    """Cast attention inputs back to the module dtype when they were upcast to fp32."""
    ...
    if warn:
        logger.warning_once(_FLOAT32_CAST_WARNING.format(target_dtype=target_dtype))
...
def _get_flash_attention_sliding_window(self) -> Optional[int]:
    """Return the active sliding-window size for this layer, if enabled."""
    if (
...
def _run_flash_attention(
    ...
    """Run FlashAttention and reshape the output back to hidden-state layout."""
    ...
    attn_output = _flash_attention_forward(
        ...
        sliding_window=_get_flash_attention_sliding_window(self),
        is_causal=self.is_causal,
        use_top_left_mask=self._flash_attn_uses_top_left_mask,
    )
    return attn_output.reshape(query_states.shape[0], q_len, self.hidden_size).contiguous()
...
def qwen_flash_attn2_forward_ExpectedAttention(
    ...
    **kwargs,
    ...
    bsz, q_len, query_states, key_states, value_states = _project_qkv(self, hidden_states)

    cos, sin = _resolve_qwen_position_embeddings(
        self,
        value_states,
        position_ids,
        position_embeddings,
    ...
    query_states, key_states, value_states = _cast_flash_attn_inputs(
        self,
        query_states,
        key_states,
        value_states,
    ...
    attn_output = _run_flash_attention(
        self,
        query_states,
        key_states,
        value_states,
...
def adakv_qwen_forward(
    ...
    image_token_positions: Optional[torch.LongTensor] = None,
    image_token_lengths: Optional[torch.LongTensor] = None,
) -> Union[Tuple, Qwen25BaseModelOutputWithPast]:
    ...
    return Qwen25BaseModelOutputWithPast(
